...
describe('ComputersComponent', () => {
  ...
  beforeEach(async(() => {
    ...
    mockComputersService = sinon.createStubInstance(ComputersService);
  ...
  it('should update the table when a Computers is added', fakeAsync(() => {
    ...
    sinon.stub(component.serviceComputers, 'addAsset').returns(new Observable<any>(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Computers is updated', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceComputers, 'updateAsset').returns(new Observable<any>(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Computers is deleted', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceComputers, 'deleteAsset').returns(new Observable<any>(observer => {
      observer.next('');
      observer.complete();
    }));
